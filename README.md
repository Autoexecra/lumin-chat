...
当前版本额外强化了面向代码仓库的开发能力：自动注入工作区摘要、Git 状态与 diff 感知、按 glob 查找文件、精确文件内替换与插入，整体更接近 GitHub Copilot 在开发工作流里的使用方式。
...
- 项目代码采用 Apache License 2.0 开源发布
- 所有源码文件统一带有版权头
- RPM 元数据与安装产物统一声明 Apache-2.0
- 提供可选的运行时许可证校验能力，默认关闭，适合交付场景按需开启
...
- 默认 `enabled=false`，开发环境不会阻塞运行
- 交付环境可将 `enabled` 改为 `true`
- 建议通过环境变量提供签名密钥，不要直接写入仓库
- 可使用 `python3 scripts/generate_license.py --expires-at 2027-03-11T00:00:00Z` 生成许可证文件
...
- [docs/license_guard.md](docs/license_guard.md)
